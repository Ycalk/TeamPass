...
class MediaTooLargeError(S3ClientException):
...
class ConfigError(S3ClientException):
    def __init__(self, message: str) -> None:
        super().__init__(message)


class MediaNotFoundError(S3ClientException):
    def __init__(self, key: str) -> None:
        super().__init__(f"Key {key} does not exist")


class MaxRetriesExceededError(S3ClientException):
