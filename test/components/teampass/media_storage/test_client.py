...
class TestS3Client:
    ...
    async def test_save_media(self, s3_client: IS3Client) -> None:
        data, file_name = self._test_media
        await s3_client.save_media(data, file_name)

    async def test_save_too_large_media(
        self, s3_client: IS3Client, media_storage_settings: MediaStorageSettings
    ) -> None:
    ...
    async def test_generate_presigned_url(self, s3_client: IS3Client) -> None:
        _, file_name = self._test_media
        url = await s3_client.generate_presigned_url(file_name)
        assert isinstance(url, str)
        assert url.startswith("http")

    async def test_generate_presigned_url_not_found(self, s3_client: IS3Client) -> None:
